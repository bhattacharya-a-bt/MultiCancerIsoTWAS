...
sss = subset(df_plot,Feature == all_res$isoQTL[which.min(all_res$isoQTL$P),]$Feature[1])
...
require(ggrepel)
top=all_res$isoQTL[which.min(all_res$isoQTL$P),]$Feature[1]
...
gwas_this = data.frame(CHROM = 'chr19',
                     POS = df_plot$Position[df_plot$Feature == 'Breast (ER-)'],
                     ID =  df_plot$SNP[df_plot$Feature == 'Breast (ER-)'],
                     P = df_plot$P[df_plot$Feature == 'Breast (ER-)'],
                     R2 = df_plot$LD_all[df_plot$Feature == 'Breast (ER-)'])

df_plot$Strip = df_plot$Feature
levels(df_plot$Strip) = c('Breast (ER-)',
                          'BABAM1 (Z = 0.6)',
                          'ENST00000599474.5 (Z = 11.7, effect)',
                          'ENST00000447614.6 (Z = -12.5)',
                          'ENST00000601436.5 (Z = 12.0)',
                          'ENST00000601938.5 (Z = -12.2)',
                          'ENST00000598188.6 (Z = -12.5)',
                          'ENST00000598382.1 (Z = -6.9)')
...
gtf = subset(gtf, 
             gene_id %in% c('ENSG00000105393'))
tx_list = unique(df_plot$Feature)

df_annotation = data.frame(gene_name = c('BABAM1'),
                           N_tx = NA,
                           N_tx_modeled = NA,
                           identifiedTx = c('ENSG00000105393'))
...
gtf_exons <- gtf %>% 
  ...
  tx_plot = ggg %>%
    ...
    geom_vline(xintercept = df_plot$Position[df_plot$Feature == 'Breast (ER-)' &
  ...
  geom_vline(xintercept = df_plot$Position[df_plot$Feature == top & 
                                             df_plot$LD_all > .8 &
                                             df_plot$Position > inGene_this$Start[1]-10 &
                                             df_plot$Position < inGene_this$End[1]+10],
             linetype = 2,
             color = 'red',
             size = .3)
...
forest_df$Feature = factor(forest_df$Feature,
                           c('BRCA (ER-)',
                             ...
                             'ENST00000598188.6',
                             'ENST00000598382.1'))
levels(forest_df$Feature)[2] = 'BABAM1'
...
ggsave(plot = tx_plot,
       filename = '/Users/abhattacharya3/OneDrive - Inside MD Anderson/Talks/DCCP Grand Rounds - March 2024/BABAM1Plot.pdf',
       width = 5.38,
       height = 4.71)
...
ggsave(plot = total,
       filename = '/Users/abhattacharya3/OneDrive - Inside MD Anderson/Fall 2024/PanCan IsoTWAS/Manuscript/Figures/Fig_BABAM1.pdf',
       height = 7,
       width = 7)
